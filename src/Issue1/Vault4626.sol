...
interface IERC20Extented is
    IERC20 // Standard IERC20 does not have an interface to get the ERC20 token decimals
{
...
contract Vault4626 {
    ...
    uint256 constant DECIMALS = 18; // THE BUG

    //track the balance of an user (user[token][value])
    mapping(address => mapping(address => uint256)) users_balances;
    ...
    function deposit(address token, uint256 value) external {
        ...
        uint256 amount_to_deposit = value * (10 ** IERC20Extented(token).decimals());
    ...
    function withdraw(address token, uint256 value) external {
        uint256 token_balance = users_balances[msg.sender][token];

        if (value > token_balance) revert Vault4626__InsufficientBalance();

        uint256 amount_to_withdraw = value * (10 ** DECIMALS);
